    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/Jugal-Jaiswal/star-agile-banking-finance.git'
                //git 'https://github.com/Jugal-Jaiswal/star-agile-banking-finance.git'
            }
		}
        stage('Maven Build') {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t Jugal-Jaiswal/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag Jugal-Jaiswal/bankapp-eta-app:${BUILD_NUMBER} Jugal-Jaiswal/bankapp-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push Jugal-Jaiswal/bankapp-eta-app:latest"
			}
		}
